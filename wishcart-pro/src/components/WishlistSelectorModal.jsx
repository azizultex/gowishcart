...
// Note: GuestEmailModal should be available from free plugin
// For now, we'll import it - if it's not available, we'll need to handle that
let GuestEmailModal;
try {
    // Try to import from free plugin's build or make it available globally
    GuestEmailModal = window.wishcartGuestEmailModal || null;
} catch (e) {
    console.warn('GuestEmailModal not available');
}
...
const WishlistSelectorModal = ({ isOpen, onClose, productId, variationId = 0, onSuccess }) => {
    ...
    if (showEmailModal && GuestEmailModal) {
        return (
            <GuestEmailModal
