...
/**
 * Sharing Handler Class (Pro Feature)
 *
 ...
 */
class WishCart_Pro_Sharing_Handler {
    ...
    public function create_share($wishlist_id, $share_type = 'link', $options = array()) {
        ...
        if (!$wishlist) {
            return new WP_Error('wishlist_not_found', __('Wishlist not found', 'wishcart-pro'));
        }
        ...
        if (false === $result) {
            return new WP_Error('db_error', __('Failed to create share', 'wishcart-pro'));
        }

        $share_id = $this->wpdb->insert_id;

        // Update analytics (if available)
        if (class_exists('WishCart_Pro_Analytics_Handler')) {
            ...
            $analytics = new WishCart_Pro_Analytics_Handler();
    ...
    public function get_share_url($share_token, $share_type = 'link') {
        // Use the shared wishlist page URL with query parameter
        if (class_exists('WishCart_Pro_Shared_Wishlist_Page')) {
            $base_url = WishCart_Pro_Shared_Wishlist_Page::get_share_url($share_token);
        } else {
            // Fallback
            $base_url = home_url('/share?token=' . urlencode($share_token));
        }
        ...
        switch ($share_type) {
            ...
            case 'email':
                $subject = __('Check out my wishlist', 'wishcart-pro');
                $body = __('I wanted to share my wishlist with you:', 'wishcart-pro') . "\n\n" . $base_url;
    ...
    public function delete_share($share_id) {
        ...
        if (false === $result) {
            return new WP_Error('db_error', __('Failed to delete share', 'wishcart-pro'));
        }
    ...
    public function send_share_email($share_id, $to_email, $message = '') {
        ...
        if (!$share) {
            return new WP_Error('share_not_found', __('Share not found', 'wishcart-pro'));
        }
        ...
        $subject = sprintf(__('%s shared a wishlist with you', 'wishcart-pro'), get_bloginfo('name'));

        // Build email content
        $email_content = sprintf(__('Hello,%s%sYou have received a shared wishlist from %s.', 'wishcart-pro'), "\n\n", "\n\n", get_bloginfo('name'));
        
        if (!empty($message)) {
            $email_content .= "\n\n" . __('Personal message:', 'wishcart-pro') . "\n" . sanitize_textarea_field($message);
        }

        $email_content .= "\n\n" . sprintf(__('Wishlist name: %s', 'wishcart-pro'), $wishlist['wishlist_name']);
        $email_content .= "\n\n" . __('View the wishlist here:', 'wishcart-pro') . "\n" . $share_url;
        ...
        return new WP_Error('email_failed', __('Failed to send email', 'wishcart-pro'));
    }
}
