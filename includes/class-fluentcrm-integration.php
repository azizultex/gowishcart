...
class GoWishCart_FluentCRM_Integration {
    ...
    public function create_or_update_contact($user_id = null, $email = null, $data = array()) {
        ...
        try {
            ...
            return new WP_Error('fluentcrm_api_not_found', __('FluentCRM API not found', 'gowishcart-wishlist-for-fluentcart-pro'));
        } catch (Exception $e) {
    ...
    public function attach_tags($contact_id, $tag_ids) {
        ...
        try {
            ...
            return new WP_Error('contact_not_found', __('Contact not found', 'gowishcart-wishlist-for-fluentcart-pro'));
        } catch (Exception $e) {
            return new WP_Error('fluentcrm_error', $e->getMessage());
        }
    }
    ...
    public function send_email($contact_id, $subject, $body, $options = array()) {
        ...
        try {
            ...
            if (!$email_address) {
                return new WP_Error('no_email', __('Contact email not found', 'gowishcart-wishlist-for-fluentcart-pro'));
            }
            ...
            if ($email_sent) {
                ...
                $error_message = $last_error ? $last_error : __('Failed to send email', 'gowishcart-wishlist-for-fluentcart-pro');
    ...
    public function sync_wishlist_user($user_id, $wishlist_data = array()) {
        ...
        if (!$settings['enabled'] || !$settings['auto_create_contacts']) {
            return new WP_Error('sync_disabled', __('Contact sync is disabled', 'gowishcart-wishlist-for-fluentcart-pro'));
        }

        $user = get_userdata($user_id);
        if (!$user) {
            return new WP_Error('user_not_found', __('User not found', 'gowishcart-wishlist-for-fluentcart-pro'));
        }
    ...
    public function create_list_if_not_exists($list_name) {
        ...
        if (empty($list_name)) {
            return new WP_Error('invalid_list_name', __('Invalid list name', 'gowishcart-wishlist-for-fluentcart-pro'));
        }
        ...
        try {
            if (class_exists('\FluentCrm\App\Models\Lists')) {
                ...
                return new WP_Error('list_creation_failed', __('Failed to create list', 'gowishcart-wishlist-for-fluentcart-pro'));
            }
